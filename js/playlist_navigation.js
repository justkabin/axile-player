(function (window, $){
	"use strict"	
	var AXILEPlaylistNavigation = function(settings){
		...
		navBackward = wrapper.find('.axile-nav-backward'),
		navForward = wrapper.find('.axile-nav-forward'),
			...
			if(navigationType == 'scroll'){
				...
				if(settings.playlistScrollType == 'mcustomscrollbar'){
					...
					if(!playlistScrollCssExistCheckDone && !checkIfCssExist('mcustomscrollbar')){
						...
						if(!AXILEUtils.relativePath(settings.mCustomScrollbar_css))var src = AXILEUtils.qualifyURL(settings.sourcePath+settings.mCustomScrollbar_css);
					...
					if(typeof mCustomScrollbar === 'undefined'){
						...
						if(!AXILEUtils.relativePath(settings.mCustomScrollbar_js))var src = AXILEUtils.qualifyURL(settings.sourcePath+settings.mCustomScrollbar_js);
					...
					playlistInner.mCustomScrollbar({
						...
						callbacks:{
						    onOverflowYNone:function(){
						        playlistInner.find('.mCSB_container').addClass('axile-mCSB_full');//hide scrollbar area if no scroll
						    }, 
						    onOverflowY:function(){
					            playlistInner.find('.mCSB_container').removeClass('axile-mCSB_full');
					        },
					...
					if(!playlistScrollCssExistCheckDone && !checkIfCssExist('perfect-scrollbar')){
						...
						if(!AXILEUtils.relativePath(settings.perfectScrollbar_css))var src = AXILEUtils.qualifyURL(settings.sourcePath+settings.perfectScrollbar_css);
					...
					if(typeof PerfectScrollbar !== 'function'){
						...
						var script = document.createElement('script');
						script.type = 'text/javascript';
						if(!AXILEUtils.relativePath(settings.perfectScrollbar_js))var src = AXILEUtils.qualifyURL(settings.sourcePath+settings.perfectScrollbar_js);
		...
		this.scrollTo = function(activePlaylistItem){
			...
			if(navigationType == 'scroll'){
				...
				var index = playlistContent.find('.axile-playlist-item').index(activePlaylistItem);
	...
	window.AXILEPlaylistNavigation = AXILEPlaylistNavigation;
