(function (window, $){
	"use strict"
	var AXILERelPagination = function (data){
	...
	playlistHolder = wrapper.find('.axile-rel-playlist-holder'),
	playlistInner = wrapper.find('.axile-rel-playlist-inner'),
	playlistContent = wrapper.find('.axile-rel-playlist-content')

	

	AXILEUtils.keysrt(breakPointArr, 'width');//sort from low to high
	...
	$(data.settings.playlistList).find('.axile-rel-playlist').each(function(){
		var playlist = $(this),
		rel_num = playlist.attr('data-rel-num'),
		len = playlist.find('.axile-playlist-item').length,
		...
		while(arr.length < rel_num){
		    ...
		    if(arr.indexOf(r) === -1){//not repeat
		    	arr.push(r);
		    	pl_items.push(playlist.find('.axile-playlist-item').eq(r).attr('data-playlist-id', playlist_id));
		    }
		}
	...
	var i, len = pl_items.length, str, item, hover_class = isMobile ? ' axile-rel-box-mobile' : '', imgsrc, title, duration;
	...
	for(i=0;i<len;i++){
		...
		str = '<div class="axile-rel-box'+hover_class+'" data-id="'+i+'" data-playlist-id="'+item.attr('data-playlist-id')+'" data-media-id="'+item.attr('data-media-id')+'"';
					if(title)str += ' title="'+title+'"';
					str += '>'+ 
                    '<div class="axile-rel-thumb">'+
                    	'<img class="axile-rel-thumbimg" src="'+imgsrc+'" alt="image" />'+ 
                    '</div>';
                    if(title)str += '<div class="axile-rel-title">'+title+'</div>';
                    if(duration)str += '<div class="axile-rel-duration">'+AXILEUtils.formatTime(duration)+'</div>';
                str += '</div>';
	...
	var prev = wrapper.find('.axile-rel-prev').on('click',function(){
	...
	var next = wrapper.find('.axile-rel-next').on('click',function(){
	...
	wrapper.find('.axile-rel-close').on('click',function(){
	...
	this.build = function(){
		...
		if(shuffleOnOpen)AXILEUtils.shuffleArray(playlistDataArr);
		...
		if(!inited){
			...
			playlistContent.find('.axile-rel-box').each(function(){
				...
				item = $(this).on('click', function(){
					...
					var lp = parent.loadPlaylist('.axile-playlist-'+playlist_id, media_id);
					if(!lp)parent.loadMedia('id',media_id);//playlist already loaded, load just video
				});

				//fade thumbs
				fadeArr.push(item.find('.axile-rel-thumbimg'));
			...
			for(z=0;z<len;z++){
				setTimeout(function(){
				   clearTimeout(this);
				   fadeArr[p].addClass('axile-visible');
				   p++;
				},50 + ( z * 50 ));
			}	
			...
			playlistContent.find('.axile-rel-box').each(function(){
				fadeArr.push($(this).find('.axile-rel-thumbimg'));
			});
	...
	function buildPaginationContainer(){
		...
		for(i = 0; i < contLen; i++){
			...
			var cont = $('<div class="axile-rel-cont"></div>');
		...
		var i = 0;
		playlistContent.find('.axile-rel-cont').each(function(){
			i++;
			$(this).width(w);
		});
		...
		if(navigationDirection == 'h'){
			...
			contSize = playlistContent.children('.axile-rel-cont').eq(0).width();
		}else{
			contSize = playlistContent.children('.axile-rel-cont').eq(0).height();
		}	

		var h = playlistContent.children('.axile-rel-cont').eq(0).height();
		...
		if(lastVisibleBoxId){//reposition to last visible box (approximately)
			...
			var cont = playlistContent.find('.axile-rel-box[data-id='+lastVisibleBoxId+']').parent();
			if(cont.find('.axile-rel-box').length == boxCount){//if all boxes inside
	...
	function destroyPaginationContainer(reset_opacity){
		...
		playlistContent.css('display','none').find('.axile-rel-box').each(function(){
			item = $(this);
			if(reset_opacity)item.find('.axile-rel-thumbimg').removeClass('axile-visible');
			playlistDataArr.push(item.detach());//save data with detach!
		})
		playlistContent.find('.axile-rel-cont').remove();
	...
	this.resizePagination = function(){
		...
		if(column != lastColumn || row != lastRow){
			...
			lastVisibleBoxId = playlistContent.children('.axile-rel-cont').eq(slideCounter).find('.axile-rel-box').eq(0).attr('data-id');
	...
	function executeSlide(){
		...
		//playlist height
		var h = playlistContent.children('.axile-rel-cont').eq(slideCounter).height();
		playlistInner.css('height', h + gutter);
	...
	window.AXILERelPagination = AXILERelPagination;
